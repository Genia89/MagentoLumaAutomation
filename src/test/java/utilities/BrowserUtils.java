...
public class BrowserUtils {
    ...
    /**
     * This method will accept WebElement as an argument,
     * and will click on that element.
     *
     * @param webElement the element on which you want to click
     */
    public static void click(WebElement webElement) {
        // wait for page to load
        waitForPageToLoad(20);
        // click on element with a timeout of 20 seconds
        clickWithTimeOut(webElement, 20);
    }

    /**
     * Retrieves the background color of the step circle for a given step number.
     *
     * This method is designed to get the color of a specific step in the multi-step checkout process,
     * such as "Start Application", "Payment Plan", or "Review". It identifies the step based on the
     * provided step number and returns the background color in hexadecimal format.
     *
     * The color of each step represents the current state of the process:
     * - Steps that have been completed will have a green color.
     * - The active step will have a blue color.
     *
     * @param stepNumber the number of the step to retrieve the color for (1 = Start Application, 2 = Payment Plan, 3 = Review)
     * @return the hexadecimal representation of the background color of the step circle (e.g., "#acf58a" for green)
     * @throws IllegalArgumentException if the provided step number is invalid (not 1, 2, or 3)
     */
    public static String getStepColor(int stepNumber, WebElement startApplicationStepCircle, WebElement paymentPlanStepCircle, WebElement reviewStepCircle) {
        WebElement stepCircle;
        // Identify which step's circle to retrieve the color for
        switch (stepNumber) {
            case 1:
                stepCircle = startApplicationStepCircle;
                break;
            case 2:
                stepCircle = paymentPlanStepCircle;
                break;
            case 3:
                stepCircle = reviewStepCircle;
                break;
            default:
                throw new IllegalArgumentException("Invalid step number: " + stepNumber);
        }
        // Retrieve and return the background color of the step circle
        return Color.fromString(stepCircle.getCssValue("background-color")).asHex();
    }
